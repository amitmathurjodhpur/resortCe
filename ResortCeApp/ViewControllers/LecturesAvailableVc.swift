...
class LecturesAvailableVc: UIViewController {
    ...
    func postOneGroup()//list of all courses in one group
    {
        ...
        DataManager.postAPIWithParameters(urlString: API.oneGroupList, jsonString: Request.OneGroupList((UserDefaults.standard.value(forKey: "authKey") as! String), getGroupId) as [String : AnyObject], success: {
            sucess in
            ActivityIndicator.shared.hide()
            let Body = sucess["body"] as? [String:Any]
            let Group = Body!["Group"] as! [String : Any]
            ...
            {
            self.nameArray = Group["course"] as! [[String : Any]]
            }
    ...
    {
        ActivityIndicator.shared.show(self.view)
        DataManager.postAPIWithParameters(urlString: API.subcategory_listing, jsonString: Request.setauthKey(UserDefaults.standard.value(forKey: "authKey") as! String) as [String : AnyObject], success: {
            sucess in
             ActivityIndicator.shared.hide()
            print(sucess)
            self.TypeArray = sucess["body"] as! [[String:Any]]
    ...
    {
        ActivityIndicator.shared.show(self.view)
        DataManager.postAPIWithParameters(urlString: API.subcategory_course_listing, jsonString: Request.SubCategoryCourseListing(UserDefaults.standard.value(forKey: "authKey") as! String, SubCategoryId) as [String : AnyObject] , success: {
            sucess in
            ActivityIndicator.shared.hide()
            print(sucess)
